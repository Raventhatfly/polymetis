...
_LIB_NAME = "libtorchscript_pinocchio.so"
_SEARCH_PATHS = []
# 1. CONDA_PREFIX/lib (installed via conda build)
if "CONDA_PREFIX" in os.environ:
    _SEARCH_PATHS.append(os.path.join(os.environ["CONDA_PREFIX"], "lib", _LIB_NAME))
# 2. Full cmake build dir (original polymetis layout: polymetis/build/)
_SEARCH_PATHS.append(os.path.abspath(os.path.join(PKG_ROOT_DIR, "../../build/torch_isolation", _LIB_NAME)))
# 3. Standalone torch_isolation build (torch_isolation/build/)
_SEARCH_PATHS.append(os.path.abspath(os.path.join(PKG_ROOT_DIR, "../../torch_isolation/build", _LIB_NAME)))
...
for _path in _SEARCH_PATHS:
    if os.path.isfile(_path):
        ...
        break
if not _loaded:
    raise OSError(f"Could not find {_LIB_NAME} in any of: {_SEARCH_PATHS}")
